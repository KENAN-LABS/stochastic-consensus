...
Every sample from a language model is a draw from a distribution, not a lookup.
Ask once and you get one trajectory through that distribution — usually a
high-probability, slightly generic one. The valuable material (the unusual
option, the failure mode nobody lists first) sits in the tail, and a single
sample rarely reaches it.

Sampling the same question many times *in genuine isolation* yields two signals:

- **Coverage** — the union of many samples is far wider than any one sample.
- **Recurrence** — an answer reached independently by 9 of 10 isolated samples is
  load-bearing; one reached once is speculative. Frequency is a cheap, honest
  confidence signal no single response can provide.

Isolation is the whole ballgame. Subagents work here precisely because they
cannot see each other — no anchoring, no groupthink, no agent politely building
on whatever the first one said. Asking one agent for 50 ideas instead gives one
trajectory with 50 items on it, heavily self-correlated. That is a long list, not
a consensus.
...
Generator agents cost roughly 40–50k tokens each. State N, M, and the lenses in
one line before spawning so the user can stop you.
...
itself. Adversarial spread surfaces failure modes a room of agreeable agents
never mentions. Assigning *topical* lenses in convergent mode is the most common
...
part. Nested spawning works, so this functions even when the skill itself runs
inside a subagent.

Name each agent's lens in the call's `description` (e.g. "Lens: caching") so the
run is legible while executing and you can see at a glance whether the lenses
were actually distinct.

Every generator prompt needs all five of these or it drifts:
...
spawning rather than improvising the wording each time.
...
- `references/failure-modes.md` — the six ways this goes wrong, and the fixes
